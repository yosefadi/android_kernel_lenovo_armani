...
/* yangjq, 20121127, For PM debug, START */
int clk_dump_info(char* buf, int* count, int enabled)
{
	struct clk *clk;
	unsigned i;
	char* p = buf;
	*count = 0;
	...
	//pr_info("%s(), %d, %d\n", __func__, __LINE__, num_msm_clocks);
	for (i = 0; i < num_msm_clocks; i++) {
		clk = msm_clocks[i].clk;
		//pr_info("%s(), %08x, %08x\n", __func__, (unsigned int)clk, (unsigned int)clk->ops->is_enabled);
		if(clk && clk->ops->is_enabled) {
			int en = (int) clk->ops->is_enabled(clk);
			if(en == enabled) {
				unsigned long rate = clk_get_rate(clk);
				p += sprintf(p, "[%d] %s (%3lu.%lu MHz)\n", *count, clk->dbg_name, rate / 1000000, (rate / 1000) % 1000);
				(*count) ++;
			}
		}
	}
...
/* yangjq, 20121127, For PM debug, END */
