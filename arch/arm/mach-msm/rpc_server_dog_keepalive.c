...
static unsigned long dog_server_last_stamp;
static int handle_rpc_call(struct msm_rpc_server *server,
			   struct rpc_request_hdr *req, unsigned len)
{
	static unsigned long count=0;

	dog_server_last_stamp = jiffies;
	//every 5 minutes to print the message
	if (!(count & 0x1f))
		pr_info("dog_keep_alive: beacon count=0x%lx,jiffies=0x%lx\n", count,dog_server_last_stamp);
