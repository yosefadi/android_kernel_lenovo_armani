...
int vreg_get_status(int subcommand, unsigned id, unsigned* on, unsigned* mv)
{
	int res;
	unsigned para1, para2 = 0xffff;

	if((VREG_CTRL_GET_STATUS != subcommand) && (VREG_CTRL_GET_LAST_SLEEP_STATUS != subcommand))
		return 0;

	para1 = (subcommand << 16) | id;
	res = msm_proc_comm(PCOM_OEM_VREG_CTRL, &para1, &para2);
	if(res >= 0) {
		*on = para1;
		*mv = para2;
		return 1;
...
void vreg_config(struct regulator_dev *rdev, unsigned on, unsigned mv)
{
	unsigned para1, para2;

	para1 = (VREG_CTRL_SET << 16) | rdev->desc->id;
	para2 = (on << 16) | mv;
	msm_proc_comm(PCOM_OEM_VREG_CTRL, &para1, &para2);
}
