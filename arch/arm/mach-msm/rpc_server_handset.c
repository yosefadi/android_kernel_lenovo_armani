...
extern int up_key_pressed;
extern int down_key_pressed;
static bool headset_in = false;
 ...
 */
extern ktime_t power_key_starttime;
static int last_jiffies = 0;
#define CONFIG_DUMP_KEY
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	printk(KERN_INFO "report_hs_key, key:%d", key);
	switch (key) {
	case KEY_POWER:
	case KEY_END:
        if (key_code != HS_REL_K) {
			...
			{
				if(up_key_pressed==1&&down_key_pressed==1&&headset_in==true)
				{
#ifdef CONFIG_DUMP_KEY					
					int* dump = NULL;
