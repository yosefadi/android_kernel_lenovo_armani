...
static ssize_t fsg_store_file_exec(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count);

static void fsg_store_file_work(struct work_struct *work){
	struct cdrom_delay_work *dw = container_of(work, struct cdrom_delay_work, work.work);
	fsg_store_file_exec(dw->dev,dw->attr,dw->buf,dw->count);
	return;
}
...
ssize_t fsg_store_file(struct device *dev, struct device_attribute *attr,
	...
	int delay_times = 0;
	if(buf != NULL&&strncmp(buf,"/system",5)){ //not iso
		return fsg_store_file_exec(dev,attr,buf,count);
	}	
	else{
		cdrom_dw.dev = dev;
		cdrom_dw.attr = attr;
		cdrom_dw.buf = buf;
		cdrom_dw.count = count;
		if(only_once == 0){
			INIT_DELAYED_WORK_DEFERRABLE(&cdrom_dw.work, fsg_store_file_work);
			schedule_delayed_work(&cdrom_dw.work, 8*HZ);
			only_once++;
		}
		else{
			if(buf == NULL)
				...
				delay_times = 8*HZ;
			cancel_delayed_work(&cdrom_dw.work);
			schedule_delayed_work(&cdrom_dw.work, delay_times);
		}
	}
	return 0;	
}
