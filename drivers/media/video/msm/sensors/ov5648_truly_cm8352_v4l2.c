			...
			if(res == 0)//capture
			{
				...
				MSM_CAMERA_I2C_BYTE_DATA);
        			msm_camera_i2c_write(
        				...
        				0x3821, 0x00,
        				MSM_CAMERA_I2C_BYTE_DATA);
                        	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3820,
                        			&rdata, MSM_CAMERA_I2C_BYTE_DATA);
                        	CDBG("%s  0: 0x3820 = 0x%x\n", __func__,rdata);

                        	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3821,
                        			&rdata, MSM_CAMERA_I2C_BYTE_DATA);
                        	CDBG("%s  0: 0x3821 = 0x%x\n", __func__,rdata);
			}
			else//preview & video
			{
				esposure_delay_en = 1;
				...
				MSM_CAMERA_I2C_BYTE_DATA);
        			msm_camera_i2c_write(
        				...
        				MSM_CAMERA_I2C_BYTE_DATA);
                        	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3820,
                        			&rdata, MSM_CAMERA_I2C_BYTE_DATA);
                        	CDBG("%s  1: 0x3820 = 0x%x\n", __func__,rdata);

                        	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3821,
                        			&rdata, MSM_CAMERA_I2C_BYTE_DATA);
                        	CDBG("%s  1: 0x3821 = 0x%x\n", __func__,rdata);
			}
    #if 0
			if(!is_first_preview)
			{
