...
//int CONFIG_INPUT_YAS_ACCELEROMETER_POSITION = 1;
...
static ssize_t yas_acc_poll_show(struct device *, struct device_attribute *, char *);
static ssize_t yas_acc_poll_store(struct device *, struct device_attribute *, const char *, size_t);
...
static void yas_acc_early_suspend(struct early_suspend *h);
static void yas_acc_early_resume(struct early_suspend *h);
#else
static int yas_acc_suspend(struct i2c_client *, pm_message_t);
static int yas_acc_resume(struct i2c_client *);
#endif
 ...
 * ---------------------------------------------------------------------------------------- */
extern int yas_acc_driver_init(struct yas_acc_driver *f);
extern int yas_acc_driver_init_kxtf9(struct yas_acc_driver *f);
static int yas_acc_core_driver_init(struct yas_acc_private_data *data)
{
    ...
    if(data->client->addr == 0x53){
        err = yas_acc_driver_init(driver);
    }
    else if(data->client->addr == 0x0F){
        err = yas_acc_driver_init_kxtf9(driver);
    }
    else{
        printk("accelerometer: data->client->addr (%x) not correct!!!\n", data->client->addr);
        return -1;
    }
    if (err != YAS_NO_ERROR) {
        kfree(driver);
    ...
    /*if(data->client->addr == 0x0F)
        CONFIG_INPUT_YAS_ACCELEROMETER_POSITION = 3;*/
...
static ssize_t yas_acc_poll_show(struct device *dev,
                                  struct device_attribute *attr,
                                  char *buf)
{
    struct input_dev *input = to_input_dev(dev);
...
static ssize_t yas_acc_poll_store(struct device *dev,
                                   struct device_attribute *attr,
                                   const char *buf,
                                   size_t count)
{
...
#if DEBUG
#if YAS_ACC_DRIVER == YAS_ACC_DRIVER_ADXL346
#define ADR_MAX (0x3d)
#else
#define ADR_MAX (0x16)
#endif
...
static void yas_acc_work_func(struct work_struct *work)
{
    ...
    if(((accel.xyz.v[0] - data->last.xyz.v[0]) > 3) || ((data->last.xyz.v[0] - accel.xyz.v[0]) > 3) ||
       ((accel.xyz.v[1] - data->last.xyz.v[1]) > 3) || ((data->last.xyz.v[1] - accel.xyz.v[1]) > 3) ||
       ((accel.xyz.v[2] - data->last.xyz.v[2]) > 3) || ((data->last.xyz.v[2] - accel.xyz.v[2]) > 3)){
...
static int yas_acc_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(client->addr == 0x53)
        pr_info("Accelerometer: adxl345\n");
    else if (client->addr == 0x0F)
        pr_info("Accelerometer: KXTF9\n");

    g_accel_registered = 1;
    ...
    data->early_suspend.level = 151;
    data->early_suspend.suspend = yas_acc_early_suspend;
    data->early_suspend.resume = yas_acc_early_resume;
...
static void yas_acc_early_suspend(struct early_suspend *h)
{
    struct yas_acc_private_data *data = container_of(h, struct yas_acc_private_data, early_suspend);
    struct yas_acc_driver *driver = data->driver;
