...
#define APDS990x_POFFSET_REG 0x1E
...
//Test for another irq processing mechanism
//#define USE_IRQ_SCHEDULE_WORK
/*
 * Structs
 */
...
static int apds990x_set_pilt(struct i2c_client *client, int threshold)
{
	...
	data->pilt = threshold;
	data->ps_hysteresis_threshold = threshold;
...
static int apds990x_set_poffset(struct i2c_client *client, int poffset)
{
	struct apds990x_data *data = i2c_get_clientdata(client);
	int ret;

	ret = i2c_smbus_write_byte_data(client, CMD_BYTE|APDS990x_POFFSET_REG, poffset);
...
static void apds990x_change_ps_threshold(struct i2c_client *client)
{
	...
	if ( (data->ps_data > data->pilt) && (data->ps_data > data->piht) ) {
		...
		printk("[APDS9900] far-to-near detected, (%d, %d, 1023, %d, %d)\n",
                data->ps_data, data->ps_hysteresis_threshold, data->control, data->ppcount);
	}
		...
		printk("[APDS9900] near-to-far detected, (%d, 0, %d, %d, %d)\n",
                data->ps_data, data->ps_threshold, data->control, data->ppcount);
	} else {
		printk("[APDS9900] data->ps_data = %d, data->pilt = %d, data->piht = %d\n", data->ps_data, data->pilt, data->piht);
		if(data->pilt == data->ps_hysteresis_threshold){
		    data->ps_detection = 0;
		    ...
		    printk("[APDS9900] modify report value, near-to-far detected, (%d, 0, %d, %d, %d)\n",
                    data->ps_data, data->ps_threshold, data->control, data->ppcount);
		} else if(data->piht == data->ps_threshold){
			data->ps_detection = 1;
			...
			printk("[APDS9900] modify report value, far-to-near detected, (%d, %d, 1023, %d, %d)\n",
                    data->ps_data, data->ps_hysteresis_threshold, data->control, data->ppcount);
		}
	}
}
...
static void apds990x_als_polling_work_handler(struct work_struct *work)
{
    ...
    if(((luxValue - data->last_als_data) > 10) || ((data->last_als_data - luxValue) > 10)){
	input_report_abs(data->input_dev_als, ABS_MISC, luxValue); // report the lux level
	input_sync(data->input_dev_als);
    data->input_dev_als->absinfo[ABS_MISC].value = -1;
    data->last_als_data = luxValue;
    }
...
retry:
	...
	if (status < 0) {
		...
		if (retry_count--) {
			...
			msleep(100);
			ret = apds990x_set_command(client, 2);	/* 2 = CMD_CLR_PS_ALS_INT */
			printk("%s, clear int (ret=%d)\n", __func__, ret);
			return;
		}
	}
    msleep(350);
...
static irqreturn_t apds990x_interrupt(int vec, void *info)
{
	...
	disable_irq_nosync(data->pdata->irq);
...
#else //USE_IRQ_SCHEDULE_WORK
static irqreturn_t apds990x_interrupt_handler(int vec, void *info)
{
	struct i2c_client *client=(struct i2c_client *)info;
	struct apds990x_data *data = i2c_get_clientdata(client);

	//Disable the irq even with a threaded interrupt handler
	disable_irq_nosync(data->pdata->irq);
...
/* assume this is ISR */
static irqreturn_t apds990x_interrupt_thread(int vec, void *info)
{
	struct i2c_client *client=(struct i2c_client *)info;
	struct apds990x_data *data = i2c_get_clientdata(client);

	printk("==> apds990x_interrupt_thread (timeout)\n");
	wake_lock_timeout(&data->prx_wake_lock, HZ / 2);
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val == 1) {
		...
		if (data->enable_ps_sensor==0) {
            ...
            apds990x_set_poffset(client, 148);
                    ...
                    struct device_attribute *attr, char *buf)
{
    ...
    printk("%s, pdata(%d), piht-pilt(%d ~ %d)\n", __func__, pdata, piht, pilt);
    mutex_unlock(&data->update_lock);
...
static DEVICE_ATTR(pilt, S_IWUSR | S_IRUGO,
           apds990x_show_pilt, apds990x_store_pilt);
...
static ssize_t apds990x_show_poffset(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct apds990x_data *data = i2c_get_clientdata(client);
...
static ssize_t apds990x_store_poffset(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    struct i2c_client *client = to_i2c_client(dev);
    unsigned long val = simple_strtoul(buf, NULL, 10);
    ...
    ret = apds990x_set_poffset(client, val);
...
static DEVICE_ATTR(poffset, S_IWUSR | S_IRUGO,
           apds990x_show_poffset, apds990x_store_poffset);

static ssize_t apds990x_show_type(struct device *dev,
                    struct device_attribute *attr, char *buf)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct apds990x_data *data = i2c_get_clientdata(client);
...
static DEVICE_ATTR(type, S_IRUGO, apds990x_show_type, NULL);
...
static int apds990x_init_client(struct i2c_client *client)
{
	...
	if (id == 0x20) {
		...
		printk("Neither APDS-9901 nor APDS-9901----(id is %x)\n", id);
		return -EIO;
	}
	...
	data->control = 0x60;
	apds990x_set_poffset(client, 148);	// positive poffset, 20
    data->poffset = 148;
				   ...
				   const struct i2c_device_id *id)
{
    ...
    //check id, whether is rpr400 or apds9930
	dev_id = i2c_smbus_read_byte_data(client, CMD_BYTE|APDS990x_ID_REG);
    if(dev_id != 0x20 && dev_id != 0x29 && dev_id != 0x39){
        kfree(data);
        return -1;
    }
    data->type = "0";
    printk("%s, dev_id(0x%x), this is apds99xx!\n", __func__, dev_id);
    ...
    data->input_dev_ps->absinfo[ABS_DISTANCE].value = -1;
...
#ifdef USE_IRQ_SCHEDULE_WORK
	err = request_threaded_irq(data->pdata->irq, apds990x_interrupt, NULL, IRQF_FORCE_RESUME | IRQF_NO_SUSPEND | IRQF_DISABLED | IRQ_TYPE_EDGE_FALLING,
		APDS990x_DRV_NAME, (void *)client);
#else
	err = request_threaded_irq(data->pdata->irq, apds990x_interrupt_handler, apds990x_interrupt_thread, IRQF_FORCE_RESUME | IRQF_NO_SUSPEND | IRQF_DISABLED | IRQ_TYPE_EDGE_FALLING,
		APDS990x_DRV_NAME, (void *)client);
#endif
	if (err) {
