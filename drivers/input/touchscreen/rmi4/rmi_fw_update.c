 ...
 */
static char *img_name1 = "Aruba-38o";
module_param(img_name1, charp, S_IRUGO | S_IWUSR);
static char *img_name2 = "Aruba-38m";
module_param(img_name2, charp, S_IRUGO | S_IWUSR);
...
static bool go_nogo(struct reflash_data *data, struct image_header *header)
{
	...
	retval = rmi_read_block(data->rmi_dev, data->f34_pdt->control_base_addr, config_id, 4);
//	if(((config_id[3] != 0x38) && !strcmp(data->product_id, "Aruba-001")) || ((config_id[3] != 0x35) && !strcmp(data->product_id, "Aruba-002"))){
		...
		struct pdt_entry *f01_pdt, struct pdt_entry *f34_pdt)
{
	...
	if(!strcmp(data.product_id, "Aruba-001")){
	snprintf(firmware_name, sizeof(firmware_name), "rmi4/%s.img",
			img_name1 ? img_name1 : (char*)data.product_id);
	}else if(!strcmp(data.product_id, "Aruba-002")){
	snprintf(firmware_name, sizeof(firmware_name), "rmi4/%s.img",
			img_name2 ? img_name2 : (char*)data.product_id);
	}
	printk("update %s touch firmware\n",firmware_name);
