...
static int bt_chip_read_proc(char *page, char **start, off_t off,
                               int count, int *eof, void *data)
{
    pr_err("cur_connv_type   =%d  \n ",cur_connv_type);
    return sprintf(page, "cur_connv_type=%d\n",cur_connv_type);
}

 void bt_proc_entry_chip(void)
{
    create_proc_read_entry("bt_chip", S_IRUGO, NULL, bt_chip_read_proc, NULL);
}


 void bt_proc_entry_chip_remove(void)
{
    remove_proc_entry("bt_chip", NULL);
}
